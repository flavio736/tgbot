...
for module_name in ALL_MODULES:
    ...
    if not imported_module.__mod_name__.lower() in IMPORTED:
        ...
        raise Exception("Não pode ter dois módulos com o mesmo nome! Por favor, mude um ")
...
def start(bot: Bot, update: Update, args: List[str]):
    if update.effective_chat.type == "private":
        ...
        update.effective_message.reply_text("Ei, oh p?")
...
def help_button(bot: Bot, update: Update):
    ...
    try:
        if mod_match:
            ...
            text = "Aqui está a ajuda para o *{}* module:\n".format(HELPABLE[module].__mod_name__) \
        ...
        if excp.message == "A mensagem não é modificada":
            pass
            ...
            LOGGER.exception("Exceção nos botões de ajuda. %s", str(query.data))
...
def get_help(bot: Bot, update: Update):
    ...
    if chat.type != chat.PRIVATE:
        ...
        update.effective_message.reply_text("Entre em contato comigo no PM para obter a lista de possíveis comandos.",
                                            reply_markup=InlineKeyboardMarkup(
        ...
        module = args[1].lower()
        text = "Aqui está a ajuda disponível para o *{}* module:\n".format(HELPABLE[module].__mod_name__) \
               + HELPABLE[module].__help__
...
def send_settings(chat_id, user_id, user=False):
    if user:
        if USER_SETTINGS:
            ...
            dispatcher.bot.send_message(user_id, "Estas são suas configurações atuais:" + "\n\n" + settings,
                                        parse_mode=ParseMode.MARKDOWN)
        ...
        if CHAT_SETTINGS:
            ...
            dispatcher.bot.send_message(user_id, "Parece que não há configurações de bate-papo disponíveis :'(\Envie isto "
                                                 "em um bate-papo em grupo, você é o administrador para encontrar as configurações atuais.",
                                        parse_mode=ParseMode.MARKDOWN)
...
def settings_button(bot: Bot, update: Update):
    ...
    try:
        if mod_match:
            ...
            chat = bot.get_chat(chat_id)
            query.message.reply_text("Olá! Existem algumas configurações para {} - vá em frente e escolha o que "
            ...
            chat = bot.get_chat(chat_id)
            query.message.reply_text(text="Olá! Existem algumas configurações para {} - vá em frente e escolha o que "
        ...
        if excp.message == "A mensagem não é modificada":
            ...
            LOGGER.exception("Exceção nos botões de configurações. %s", str(query.data))
...
def donate(bot: Bot, update: Update):
    ...
    if chat.type == "private":
        ...
        if OWNER_ID != 254318997 and DONATION_LINK:
            update.effective_message.reply_text("Você também pode doar para a pessoa que atualmente está me executando "
        ...
        try:
            ...
            update.effective_message.reply_text("Eu já te fiz doar para o meu criador!")
        except Unauthorized:
            update.effective_message.reply_text("Entre em contato primeiro no PM para obter informações sobre doações.")
...
def migrate_chats(bot: Bot, update: Update):
    ...
    LOGGER.info("Migrando de %s, to %s", str(old_chat), str(new_chat))
...
def process_update(self, update):
    ...
    if isinstance(update, TelegramError):
        try:
            ...
            self.logger.exception('Um erro não detectado foi levantado durante o tratamento do erro ')
        return
